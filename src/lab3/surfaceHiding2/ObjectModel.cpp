...
std::vector<Face3D>& ObjectModel::getFaces() {
  return faces;
}
...
glm::vec3 iVecToGlm(IVectorPtr v) {
  return glm::vec3(v->get(0), v->get(1), v->get(2));
}

void ObjectModel::determineFaceVisibilities1(IVectorPtr eye) {
  for (auto &face : faces) {
    face.visible = glm::dot(face.surface, glm::vec4(iVecToGlm(eye), 1.0f)) > 0.0;
  }
}

void ObjectModel::determineFaceVisibilities2(IVectorPtr eye_) {
  for (auto &face : faces) {
    // calculate center
    glm::vec3 eye = iVecToGlm(eye_);
    glm::vec3 c;
    for (int i = 0; i < 3; i++) {
      c += vertices[face[i]].toVec();
    }
    ...
    face.visible = glm::dot(face.getNormal(), e) > 0;
  }
}
