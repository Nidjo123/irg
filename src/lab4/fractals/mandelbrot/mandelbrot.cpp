...
void mouseFunc(int button, int state, int x, int y) {
  ...
  std::complex<double> min(umin, vmin);
  std::complex<double> max(umax, vmax);
  ...
  const double width = scale * std::abs(max.real() - min.real());
  const double height = scale * std::abs(max.imag() - min.imag());
  
  umin = c.real() - width / 2.0;
  umax = c.real() + width / 2.0;

  vmin = c.imag() - height / 2.0;
  vmax = c.imag() + height / 2.0;

  std::cout << "Clicked at (" << x << ", " << y << ") which is complex number " << c << std::endl;
